...
export function areSpeakersApproxEqual(oldSpeakers: HMSSpeaker[], newSpeakers: HMSSpeaker[]): boolean {
    if (oldSpeakers.length !== newSpeakers.length) {  // number of peers changed
        return false
    }
    const oldAudioLevels: {[k: string]: number} = oldSpeakers.reduce((map: {[k: string]: number}, speaker) => {
