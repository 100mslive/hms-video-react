...
export const StyledMessageModal = ({
	...
	return (
		<div className={styler('root')} aria-labelledby="modal-title" role="dialog" aria-modal="true">
			{show && (
						...
						<div className={styler('boxTransition')}>
							<div className={styler('boxRoot')}>
								<div className={styler('header')}>
									...
									<div
										className={`${modalType === 'feedback' ? styler('hide') : styler('closeRoot')}`}
									>
										<Button variant="no-fill" onClick={onClose} iconOnly size="sm">
											<CloseIcon />
										</Button>
									</div>
								</div>
...
export const MessageModal = withClasses<MessageModalClasses | undefined>(defaultClasses, 'MessageModal')<
	StyledMessageModalProps
>(StyledMessageModal);
